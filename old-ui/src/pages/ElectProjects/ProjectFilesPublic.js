...
import { FileElectProjectTypePublic } from "models/types/file-elect-project-type-public"

export const ProjectFilesPublic = ({ rowData, mainProps, setErrorMessage }) => {
  ...
  useEffect(() => {
    setFileElectProjectTypes(enumToArray(FileElectProjectTypePublic))
  }, [])
  ...
  const saveToDatabase = async (values) => {
    const { onAddFileElectProjectSingle } = mainProps
     ...
     const fileName = FileElectProjectTypePublic[values.fileTypeEnum]
          ...
          <Box>
              <Grid container spacing={1}>
                    ...
                    >
                      {
                        fileElectProjectTypes?.map(row => {
                          {/* if (
                            !lstElectProjectFiles.find(f => f.fileTypeEnum === row.id)
                          ) */}
                            return (
                ...
                <FilePondUpload
                  ...
                  labelText={`${mainProps.t("enums." + FileElectProjectTypePublic[formInput.fileTypeEnum])} را اینجا بکشید`}
                  waterMark={false}
                />
              </Grid>
